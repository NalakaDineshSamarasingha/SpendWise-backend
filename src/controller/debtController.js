...
exports.createNewDebt = async (req,res) =>{
    try {
      ...
      // Optional: validate user has an account (owner or member)
      const uid = new mongoose.Types.ObjectId(userId);
      const account = await Account.findOne({ $or: [{ userid: uid }, { members: uid }] }).select('_id');
      if (!account) return res.status(404).json({ message: 'Account not found for user.' });
...
exports.getAllDebt = async (req,res) => {
  try {
      ...
      if (!userId) return res.status(400).json({ message: 'Authenticated user not found.' });
      const uid = new mongoose.Types.ObjectId(userId);
      const debts = await Debt.find({ $or: [ { userid: uid }, { person: uid } ] })
        .sort({ date: -1 })
        .lean();
      res.json(debts);
    } catch (error) {
